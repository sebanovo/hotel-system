...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
                    ...
                    <tr>
                        ...
                        <td>
                            <x-adminlte-button label="" theme="primary" icon="fa fa-lg fa-fw fa-pen"
                                ...
                                data-target="#modalUpdatearticulo{{ $articulo->id }}" />
                            <x-adminlte-modal id="modalUpdatearticulo{{ $articulo->id }}" title="Actualizar articulo"
                                theme="primary" icon="fas fa-bolt" size='lg' disable-animations>
                                <form action="{{ route('articulos.update', $articulo) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
                                        <x-adminlte-input name="nombre" label="Nombre" placeholder="nombre articulo"
                            ...
                            <form style="display : inline" action="{{ route('articulos.destroy', $articulo) }}"
            ...
            {{-- Themed --}}
            <x-adminlte-modal id="modalPurple" title="Nuevo articulo" theme="primary" icon="fas fa-bolt" size='lg'
                disable-animations>
                <form class="form-crear" action="{{ route('articulos.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        <x-adminlte-input name="nombre" label="Nombre" placeholder="nombre articulo"
                            fgroup-class="col-md-6" disable-feedback />
